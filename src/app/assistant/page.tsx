...
export default function AssistantPage() {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', text: "Sorry, I'm having trouble connecting to the safety server right now." }]);
    } finally {
      setIsTyping(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-200/80 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden flex flex-col h-[650px] border border-gray-100">
        ...
        <div className="bg-[#6d28d9] p-4 text-white flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-purple-200">Powered by active community review database</p>
            </div>
          </div>
          <button onClick={() => router.back()} className="text-purple-200 hover:text-white transition-colors">
            ...
            <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {m.role === 'bot' && (
                <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-[#6d28d9]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </div>
              )}
              <div className={`p-4 max-w-[85%] whitespace-pre-wrap ${
                m.role === 'bot' 
        ...
        <div className="bg-white px-5 pb-3 flex flex-wrap gap-2">
          {["Tell me about Rohini", "How safe is Connaught Place?", "Metrics for Bandra", "Safest region?"].map((chip) => (
            <button 
              key={chip} 
